...
const ControlPanel = ({
  ...
  return (
    ...
    {/* Mobile view */}
    <div className="md:hidden flex flex-col p-2 space-y-2 z-30">
      <div className="flex justify-between items-center">
        <div className={`flex items-center gap-2 ${!isAutoPlay ? "opacity-50 pointer-events-none" : ""}`}>
          ...
          <Timer size={16} />
          <span className="min-w-[2.5rem] text-center text-sm">{intervalTime}초</span>
          <button
            onClick={() => adjustInterval(1)}
            className="p-1 hover:bg-gray-100 rounded"
            disabled={!isAutoPlay}
      ...
      <div className="flex gap-2">
        ...
        >
          {language === "kor" ? (isRandomOrder ? "랜덤 재생" : "순차 재생") : (isRandomOrder ? "Random Play" : "Sequential Play")}
        </button>
      </div>
    </div>    </div>
  );
